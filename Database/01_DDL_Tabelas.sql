...
CREATE TABLE TBL_EQUIPAMENTO (
    ...
    CONSTRAINT CK_EQUIP_CONDITION  CHECK (CONDITION_STATUS IN ('EXCELLENT','GOOD','FAIR','POOR','NO_BATTERY','N/A')),
    CONSTRAINT CK_EQUIP_STATUS     CHECK (STATUS IN ('IN_STOCK','AVAILABLE','SHIPPED','SOLD','SCRAPPED','IN_REPAIR','RESERVED','OTHER'))
);

CREATE INDEX IDX_EQUIP_MFR_MODEL ON TBL_EQUIPAMENTO (MARCA, MODEL);
