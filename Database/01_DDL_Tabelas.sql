...
CREATE TABLE TBL_EQUIPAMENTO (
    ...
    CONSTRAINT CK_CONDITION     CHECK (CONDITION_STATUS IN ('EXCELLENT','GOOD','FAIR','POOR','NO_BATTERY','N/A')),
    CONSTRAINT CK_STATUS        CHECK (STATUS IN ('IN_STOCK','LISTED','SOLD','SHIPPED','UPGRADING','BROKEN'))
);

CREATE INDEX IDX_EQUIP_MFR_MODEL ON TBL_EQUIPAMENTO (MANUFACTURER, MODEL);
...
CREATE TABLE TBL_EQUIPAMENTO_UPGRADE (
    ...
    CONSTRAINT CK_SOURCE       CHECK (SOURCE_ORIGEM IN ('REMESSA_EXTRA','NEW_PURCHASE','TRANSFERRED','WARRANTY','OTHER'))
);
