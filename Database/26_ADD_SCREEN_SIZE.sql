-- ============================================================================
-- GBS Inventory - Migration 26
-- Adiciona TBL_EQUIPAMENTO.SCREEN_SIZE (ex.: 14", 15.6", 17") e a expoe em
-- PACK_EQUIPAMENTO:
--   * PROC_INSERT / PROC_UPSERT_EQUIPAMENTO: novo parametro final
--     P_SCREEN_SIZE ... DEFAULT NULL (chamadores antigos continuam validos).
--     No UPSERT, NULL/vazio preserva o valor existente.
--   * PROC_SELECT_UID / _FILTRO / _ID / _SERIAL: retornam E.SCREEN_SIZE.
-- Idempotente: se a coluna ja existir (ORA-01430) o ALTER e ignorado.
-- ============================================================================

DECLARE
    e_col_exists EXCEPTION;
    PRAGMA EXCEPTION_INIT(e_col_exists, -1430);
BEGIN
    EXECUTE IMMEDIATE 'ALTER TABLE TBL_EQUIPAMENTO ADD (SCREEN_SIZE VARCHAR2(20))';
EXCEPTION
    WHEN e_col_exists THEN NULL;
END;
/

CREATE OR REPLACE PACKAGE PACK_EQUIPAMENTO AS
    FUNCTION FUNC_PROXIMO RETURN NUMBER;
    FUNCTION FUNC_EXISTE_UID(P_INTERNAL_UID IN VARCHAR2) RETURN NUMBER;
    FUNCTION FUNC_STATUS_DESCRICAO(P_STATUS IN VARCHAR2) RETURN VARCHAR2;

    PROCEDURE PROC_INSERT(
        P_INTERNAL_UID       IN VARCHAR2,
        P_SERIAL_NUMBER      IN VARCHAR2,
        P_MODELO             IN VARCHAR2,
        P_MARCA              IN VARCHAR2,
        P_PROCESSADOR        IN VARCHAR2,
        P_RAM_GB             IN NUMBER,
        P_STORAGE_GB         IN NUMBER,
        P_CONDITION_STATUS   IN VARCHAR2,
        P_STATUS_EQUIPAMENTO IN VARCHAR2,
        P_OBSERVACAO         IN VARCHAR2,
        P_BATTERY_CHECK      IN VARCHAR2 DEFAULT NULL,
        P_SOURCE_BATCH       IN VARCHAR2 DEFAULT NULL,
	P_SCREEN_SIZE IN VARCHAR2 DEFAULT NULL
    );

    PROCEDURE PROC_UPDATE_STATUS(
        P_INTERNAL_UID IN VARCHAR2,
        P_STATUS IN VARCHAR2
    );

    PROCEDURE PROC_UPSERT_EQUIPAMENTO(
        P_INTERNAL_UID IN VARCHAR2,
        P_SERIAL_NUMBER IN VARCHAR2,
        P_MODEL IN VARCHAR2,
        P_MARCA IN VARCHAR2,
        P_PROCESSADOR IN VARCHAR2,
        P_RAM_GB IN NUMBER,
        P_STORAGE_GB IN NUMBER,
        P_CONDITION_STATUS IN VARCHAR2,
        P_STATUS IN VARCHAR2,
        P_OBSERVACAO IN VARCHAR2,
	P_SCREEN_SIZE IN VARCHAR2 DEFAULT NULL
    );

    PROCEDURE PROC_SELECT_UID(
        P_INTERNAL_UID IN VARCHAR2,
        P_CURSOR OUT SYS_REFCURSOR
    );

    PROCEDURE PROC_SELECT_FILTRO(
        P_FILTRO IN VARCHAR2,
        P_STATUS_EQUIPAMENTO IN VARCHAR2,
        P_CURSOR OUT SYS_REFCURSOR
    );

    PROCEDURE PROC_DASHBOARD(
        P_CURSOR OUT SYS_REFCURSOR
    );

    PROCEDURE PROC_SELECT_ID(
        P_ID IN NUMBER,
        P_CURSOR OUT SYS_REFCURSOR
    );

    PROCEDURE PROC_SELECT_SERIAL(
        P_SERIAL IN VARCHAR2,
        P_CURSOR OUT SYS_REFCURSOR
    );

    PROCEDURE PROC_RESUMO_WHATSAPP(
        P_CURSOR OUT SYS_REFCURSOR
    );
END PACK_EQUIPAMENTO;
/

CREATE OR REPLACE PACKAGE BODY PACK_EQUIPAMENTO AS

-- ----------------------------------------------------------------------------
FUNCTION FUNC_PROXIMO RETURN NUMBER IS
    V_ID NUMBER;
BEGIN
    SELECT SEQ_EQUIPAMENTO.NEXTVAL INTO V_ID FROM DUAL;
    RETURN V_ID;
END;

-- ----------------------------------------------------------------------------
FUNCTION FUNC_EXISTE_UID(P_INTERNAL_UID IN VARCHAR2) RETURN NUMBER IS
    V_COUNT NUMBER;
BEGIN
    SELECT COUNT(*) INTO V_COUNT
      FROM TBL_EQUIPAMENTO
     WHERE INTERNAL_UID = P_INTERNAL_UID;
    RETURN V_COUNT;
EXCEPTION
    WHEN OTHERS THEN RETURN 0;
END;

-- ----------------------------------------------------------------------------
FUNCTION FUNC_STATUS_DESCRICAO(P_STATUS IN VARCHAR2) RETURN VARCHAR2 IS
BEGIN
    RETURN CASE P_STATUS
               WHEN 'IN_STOCK'  THEN 'In Stock'
               WHEN 'AVAILABLE' THEN 'Available'
               WHEN 'SHIPPED'   THEN 'Shipped'
               WHEN 'SOLD'      THEN 'Sold'
               WHEN 'SCRAPPED'  THEN 'Scrapped'
               WHEN 'IN_REPAIR' THEN 'In Repair'
               WHEN 'RESERVED'  THEN 'Reserved'
               WHEN 'OTHER'     THEN 'Other'
               ELSE P_STATUS
           END;
END;

-- ----------------------------------------------------------------------------
PROCEDURE PROC_INSERT(
    P_INTERNAL_UID       IN VARCHAR2,
    P_SERIAL_NUMBER      IN VARCHAR2,
    P_MODELO             IN VARCHAR2,
    P_MARCA              IN VARCHAR2,
    P_PROCESSADOR        IN VARCHAR2,
    P_RAM_GB             IN NUMBER,
    P_STORAGE_GB         IN NUMBER,
    P_CONDITION_STATUS   IN VARCHAR2,
    P_STATUS_EQUIPAMENTO IN VARCHAR2,
    P_OBSERVACAO         IN VARCHAR2,
    P_BATTERY_CHECK      IN VARCHAR2 DEFAULT NULL,
    P_SOURCE_BATCH       IN VARCHAR2 DEFAULT NULL,
    P_SCREEN_SIZE IN VARCHAR2 DEFAULT NULL
) IS
BEGIN
    INSERT INTO TBL_EQUIPAMENTO
               (ID_EQUIPAMENTO, INTERNAL_UID, SERIAL_NUMBER, MODEL, MARCA,
                PROCESSADOR, RAM_GB, STORAGE_GB, CONDITION_STATUS, STATUS,
                OBSERVACAO, BATTERY_CHECK, SOURCE_BATCH, SCREEN_SIZE, DATA_CADASTRO)
        VALUES (SEQ_EQUIPAMENTO.NEXTVAL, P_INTERNAL_UID, P_SERIAL_NUMBER,
                P_MODELO, P_MARCA, P_PROCESSADOR, NVL(P_RAM_GB, 0), NVL(P_STORAGE_GB, 0),
                NVL(NULLIF(TRIM(P_CONDITION_STATUS), ''), 'GOOD'),
                NVL(P_STATUS_EQUIPAMENTO, 'IN_STOCK'),
                P_OBSERVACAO, NULLIF(TRIM(P_BATTERY_CHECK), ''),
                NULLIF(TRIM(P_SOURCE_BATCH), ''), NULLIF(TRIM(P_SCREEN_SIZE), ''), SYSDATE);
END;

-- ----------------------------------------------------------------------------
PROCEDURE PROC_UPDATE_STATUS(P_INTERNAL_UID IN VARCHAR2, P_STATUS IN VARCHAR2) IS
BEGIN
    UPDATE TBL_EQUIPAMENTO
       SET STATUS = P_STATUS,
           DATA_ATUALIZACAO = SYSDATE
     WHERE INTERNAL_UID = P_INTERNAL_UID;
END;

-- ----------------------------------------------------------------------------
PROCEDURE PROC_UPSERT_EQUIPAMENTO(
    P_INTERNAL_UID  IN VARCHAR2,
    P_SERIAL_NUMBER IN VARCHAR2,
    P_MODEL         IN VARCHAR2,
    P_MARCA         IN VARCHAR2,
    P_PROCESSADOR   IN VARCHAR2,
    P_RAM_GB        IN NUMBER,
    P_STORAGE_GB    IN NUMBER,
    P_CONDITION_STATUS IN VARCHAR2,
    P_STATUS        IN VARCHAR2,
    P_OBSERVACAO    IN VARCHAR2,
    P_SCREEN_SIZE IN VARCHAR2 DEFAULT NULL
) IS
    V_COUNT NUMBER;
BEGIN
    SELECT COUNT(*) INTO V_COUNT
      FROM TBL_EQUIPAMENTO
     WHERE INTERNAL_UID = P_INTERNAL_UID;

    IF V_COUNT = 0 THEN
        INSERT INTO TBL_EQUIPAMENTO
                   (ID_EQUIPAMENTO, INTERNAL_UID, SERIAL_NUMBER, MODEL, MARCA,
                    PROCESSADOR, RAM_GB, STORAGE_GB, CONDITION_STATUS, STATUS, OBSERVACAO, SCREEN_SIZE, DATA_CADASTRO)
            VALUES (SEQ_EQUIPAMENTO.NEXTVAL, P_INTERNAL_UID, P_SERIAL_NUMBER,
                    P_MODEL, P_MARCA, P_PROCESSADOR, NVL(P_RAM_GB, 0), NVL(P_STORAGE_GB, 0),
                    NVL(NULLIF(TRIM(P_CONDITION_STATUS), ''), 'GOOD'),
                    NVL(P_STATUS, 'IN_STOCK'), P_OBSERVACAO, NULLIF(TRIM(P_SCREEN_SIZE), ''), SYSDATE);
    ELSE
        UPDATE TBL_EQUIPAMENTO
           SET MODEL            = P_MODEL,
               MARCA            = P_MARCA,
               PROCESSADOR      = P_PROCESSADOR,
               RAM_GB           = NVL(P_RAM_GB, RAM_GB),
               STORAGE_GB       = NVL(P_STORAGE_GB, STORAGE_GB),
               CONDITION_STATUS = NVL(NULLIF(TRIM(P_CONDITION_STATUS), ''), 'GOOD'),
               OBSERVACAO       = P_OBSERVACAO,

               SCREEN_SIZE = NVL(NULLIF(TRIM(P_SCREEN_SIZE), ''), SCREEN_SIZE),
               DATA_ATUALIZACAO = SYSDATE
         WHERE INTERNAL_UID = P_INTERNAL_UID;
    END IF;
END;

-- ----------------------------------------------------------------------------
PROCEDURE PROC_SELECT_UID(P_INTERNAL_UID IN VARCHAR2, P_CURSOR OUT SYS_REFCURSOR) IS
BEGIN
    OPEN P_CURSOR FOR
         SELECT E.ID_EQUIPAMENTO, E.INTERNAL_UID, E.SERIAL_NUMBER,
                E.MARCA, E.MODEL, E.PROCESSADOR, E.RAM_GB, E.STORAGE_GB, E.SCREEN_SIZE,
                E.CONDITION_STATUS, E.STATUS, E.OBSERVACAO, E.DATA_CADASTRO, E.DATA_ATUALIZACAO,
                FUNC_STATUS_DESCRICAO(E.STATUS) AS STATUS_DESCRICAO,
                (SELECT COUNT(*) FROM TBL_EQUIPAMENTO_UPGRADE U
                  WHERE U.ID_EQUIPAMENTO = E.ID_EQUIPAMENTO) AS TOTAL_UPGRADES
           FROM TBL_EQUIPAMENTO E
          WHERE UPPER(TRIM(E.INTERNAL_UID)) = UPPER(TRIM(P_INTERNAL_UID));
END;

-- ----------------------------------------------------------------------------
PROCEDURE PROC_SELECT_FILTRO(
    P_FILTRO             IN VARCHAR2,
    P_STATUS_EQUIPAMENTO IN VARCHAR2,
    P_CURSOR             OUT SYS_REFCURSOR
) IS
    V_PESQ VARCHAR2(200);
BEGIN
    V_PESQ := '%' || UPPER(TRIM(NVL(P_FILTRO, ''))) || '%';
    OPEN P_CURSOR FOR
         SELECT E.ID_EQUIPAMENTO, E.INTERNAL_UID, E.SERIAL_NUMBER,
                E.MARCA, E.MODEL, E.PROCESSADOR, E.RAM_GB, E.STORAGE_GB, E.SCREEN_SIZE,
                E.CONDITION_STATUS, E.STATUS, E.OBSERVACAO, E.DATA_CADASTRO, E.DATA_ATUALIZACAO,
                FUNC_STATUS_DESCRICAO(E.STATUS) AS STATUS_DESCRICAO,
                E.SOURCE_BATCH
           FROM TBL_EQUIPAMENTO E
          WHERE (P_FILTRO IS NULL
                 OR UPPER(E.INTERNAL_UID)  LIKE V_PESQ
                 OR UPPER(E.SERIAL_NUMBER) LIKE V_PESQ
                 OR UPPER(E.MODEL)         LIKE V_PESQ
                 OR UPPER(E.MARCA)         LIKE V_PESQ
                 OR UPPER(E.PROCESSADOR)   LIKE V_PESQ)
            AND (P_STATUS_EQUIPAMENTO IS NULL OR UPPER(E.STATUS) = UPPER(P_STATUS_EQUIPAMENTO))
          ORDER BY E.DATA_CADASTRO DESC;
END;

-- ----------------------------------------------------------------------------
PROCEDURE PROC_DASHBOARD(P_CURSOR OUT SYS_REFCURSOR) IS
BEGIN
    OPEN P_CURSOR FOR
         SELECT
            (SELECT COUNT(*) FROM TBL_EQUIPAMENTO)                              AS TOTAL_UNIDADES,
            (SELECT COUNT(*) FROM TBL_EQUIPAMENTO WHERE STATUS = 'IN_STOCK')    AS EM_ESTOQUE,
            (SELECT COUNT(*) FROM TBL_EQUIPAMENTO_UPGRADE
              WHERE DATA_UPGRADE >= TRUNC(SYSDATE) - 30)                        AS UPGRADES_30D,
            (SELECT COUNT(*) FROM TBL_REMESSA
              WHERE STATUS_REMESSA NOT IN ('DELIVERED','RETURNED'))              AS REMESSAS_ATIVAS
           FROM DUAL;
END;

-- ----------------------------------------------------------------------------
PROCEDURE PROC_SELECT_ID(P_ID IN NUMBER, P_CURSOR OUT SYS_REFCURSOR) IS
BEGIN
    OPEN P_CURSOR FOR
         SELECT E.ID_EQUIPAMENTO, E.INTERNAL_UID, E.SERIAL_NUMBER,
                E.MARCA, E.MODEL, E.PROCESSADOR, E.RAM_GB, E.STORAGE_GB, E.SCREEN_SIZE,
                E.CONDITION_STATUS, E.STATUS, E.OBSERVACAO, E.DATA_CADASTRO, E.DATA_ATUALIZACAO,
                FUNC_STATUS_DESCRICAO(E.STATUS) AS STATUS_DESCRICAO,
                (SELECT COUNT(*) FROM TBL_EQUIPAMENTO_UPGRADE U
                  WHERE U.ID_EQUIPAMENTO = E.ID_EQUIPAMENTO) AS TOTAL_UPGRADES
           FROM TBL_EQUIPAMENTO E
          WHERE E.ID_EQUIPAMENTO = P_ID;
END;

-- ----------------------------------------------------------------------------
PROCEDURE PROC_SELECT_SERIAL(P_SERIAL IN VARCHAR2, P_CURSOR OUT SYS_REFCURSOR) IS
BEGIN
    OPEN P_CURSOR FOR
         SELECT E.ID_EQUIPAMENTO, E.INTERNAL_UID, E.SERIAL_NUMBER,
                E.MARCA, E.MODEL, E.PROCESSADOR, E.RAM_GB, E.STORAGE_GB, E.SCREEN_SIZE,
                E.CONDITION_STATUS, E.STATUS, E.OBSERVACAO, E.DATA_CADASTRO, E.DATA_ATUALIZACAO,
                FUNC_STATUS_DESCRICAO(E.STATUS) AS STATUS_DESCRICAO,
                (SELECT COUNT(*) FROM TBL_EQUIPAMENTO_UPGRADE U
                  WHERE U.ID_EQUIPAMENTO = E.ID_EQUIPAMENTO) AS TOTAL_UPGRADES
           FROM TBL_EQUIPAMENTO E
          WHERE UPPER(TRIM(E.SERIAL_NUMBER)) = UPPER(TRIM(P_SERIAL));
END;

-- ----------------------------------------------------------------------------
PROCEDURE PROC_RESUMO_WHATSAPP(P_CURSOR OUT SYS_REFCURSOR) IS
BEGIN
    OPEN P_CURSOR FOR
         SELECT MARCA, MODEL, COUNT(*) AS QTD
           FROM TBL_EQUIPAMENTO
          WHERE STATUS = 'IN_STOCK'
          GROUP BY MARCA, MODEL
          ORDER BY COUNT(*) DESC;
END;

END PACK_EQUIPAMENTO;
/

SHOW ERRORS

SELECT OBJECT_TYPE, STATUS
  FROM USER_OBJECTS
 WHERE OBJECT_NAME = 'PACK_EQUIPAMENTO'
 ORDER BY OBJECT_TYPE;