-- ============================================================================
-- GBS Inventory - Migration 27
-- Adiciona MODEL em TBL_COMPONENT (ex: "OptiPlex 3080", "EliteDesk 800 G6").
-- PROC_INSERT ganha P_MODEL; selects trazem MODEL; busca do filtro inclui MODEL.
-- Idempotente.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1. Adiciona coluna MODEL (se nao existir)
-- ----------------------------------------------------------------------------
DECLARE V_N NUMBER;
BEGIN
    SELECT COUNT(*) INTO V_N FROM USER_TAB_COLUMNS
     WHERE TABLE_NAME = 'TBL_COMPONENT' AND COLUMN_NAME = 'MODEL';
    IF V_N = 0 THEN
        EXECUTE IMMEDIATE 'ALTER TABLE TBL_COMPONENT ADD (MODEL VARCHAR2(100))';
        DBMS_OUTPUT.PUT_LINE('OK: coluna MODEL adicionada');
    ELSE
        DBMS_OUTPUT.PUT_LINE('SKIP: MODEL ja existe');
    END IF;
END;
/
